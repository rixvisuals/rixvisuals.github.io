...
import { useRandomDrivePhotos } from './services/drivePhotos';
...
export default function App() {
  ...
  // Dynamically load Google Drive portfolio photos, pick 2 random photos on load, and shuffle every 5 seconds
  const { pair: randomPair, loading: photosLoading } = useRandomDrivePhotos();
                ...
                >
                  ...
                  <section className="py-20 bg-white border-t border-neutral-100" id="who-is-section">
                    <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center">
                      ...
                      {/* Left Column: Overlapping images or circular loading state */}
                      <div className="lg:col-span-6 flex justify-center lg:justify-start">
                        {photosLoading || !randomPair ? (
                          <div className="relative w-full max-w-[480px] h-[450px] sm:h-[550px] flex flex-col items-center justify-center bg-neutral-50 rounded-2xl border border-neutral-100 p-8 shadow-inner">
                            <div className="w-10 h-10 border-3 border-neutral-200 border-t-black rounded-full animate-spin mb-4" />
                            <span className="font-mono text-[10px] tracking-[0.25em] text-neutral-400 uppercase text-center font-medium">
                              LOADING PORTFOLIO CAPTURES...
                            </span>
                          </div>
                        ) : (
                          <div className="relative w-full max-w-[480px] h-[450px] sm:h-[550px]">
                            {/* Back Image (Shuffled Portfolio Image 2 - rotates every 5s) */}
                            <div className="absolute right-0 top-4 w-[60%] aspect-[3/4] overflow-hidden rounded-2xl shadow-xl border border-neutral-100 z-10 transition-transform hover:scale-102 bg-neutral-100">
                              <img
                                key={randomPair.rightImg}
                            ...
                            {/* Front Image (Shuffled Portfolio Image 1 - rotates every 5s) */}
                            <div className="absolute left-0 bottom-4 w-[65%] aspect-[3/4] overflow-hidden rounded-2xl shadow-2xl border-4 border-white z-20 transition-transform hover:scale-102 bg-neutral-100">
                              <img
                                key={randomPair.leftImg}
                            ...
                            {/* Stellenbosch geolocation signature coordinate badge */}
